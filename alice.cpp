...
void* InitShm() {
    ...
    std::atomic<std::uint64_t>* magic = (std::atomic<std::uint64_t>*)(((char*)ret) + MAGIC_OFFSET);
    auto t = BOB_MAGIC;
    while(!magic->compare_exchange_weak(t, ALICE_MAGIC)) { // wait for bob init
        usleep(1000);
        t = BOB_MAGIC;
    }
    std::cout << "Alice init" << std::endl;
    return ret;
}

void* shm_ptr = InitShm();

void send(const Message *message)
{
    memcpy(GetAliceDataPtr(), (void*)message, message->size);
